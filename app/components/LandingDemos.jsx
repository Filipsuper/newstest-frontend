...
function Sparkline({ values, color, width = 600, height = 190, dots = [] }) {
    ...
    return (
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto block" aria-hidden="true">
            ...
            <line x1="0" y1={height * 0.32} x2={width} y2={height * 0.32} stroke="var(--company-muted-line)" strokeWidth="1" strokeDasharray="5 5" opacity="0.5" />
...
const card = "product-demo bg-foreground rounded-xl select-none pointer-events-none";
        ...
        <div className={`${card} p-6`} aria-hidden="true">
            <div className="flex flex-row justify-between items-start mb-5">
                ...
                <div className="text-right font-sans">
                    ...
                    <p className="text-xs font-semibold market-negative">−3,90 (−1,06%)</p>
                </div>
            </div>
            ...
            <Sparkline
                ...
                dots={[{ at: 8, color: "var(--color-accent)" }, { at: 24, color: "var(--color-secondary)" }]}
            />
            ...
            <div className="mt-4 pt-3">
                <div className="flex flex-row items-center gap-2 font-sans text-[11px] mb-1">
                    ...
                    <Chip color="news-tag--neutral">Rapport</Chip>
                </div>
        ...
        <div className={`${card} p-6`} aria-hidden="true">
            ...
            <div className="flex flex-col gap-1.5 mb-4">
                {[
                    ...
                    <div key={idx} className="flex items-center gap-2 text-text text-xs font-bold font-serif w-fit">
                        <svg className="w-1.5 h-1.5 fill-secondary shrink-0" viewBox="0 0 8 8"><circle cx="4" cy="4" r="4" /></svg>
                        {line}
                    </div>
                ))}
            </div>
            <div className="flex flex-row items-center gap-4 font-sans text-xs mb-4">
                ...
                <div className="pl-4">
                    ...
                    <p className="font-bold text-text">3 287,68 <span className="market-positive font-semibold">+0,31%</span></p>
                </div>
            </div>
...
export function DemoNewsFeed() {
    const items = [
        ...
        {
            time: "09:52",
            chips: [["news-tag--amber", "Insynshandel"], ["news-tag--blue", "Förvärv"]],
        ...
        {
            time: "09:00",
            chips: [["news-tag--neutral", "Rapport"]],
            company: "Volvo Car B",
        ...
        <div className={`${card} p-6`} aria-hidden="true">
            <div className="flex flex-row justify-between items-center mb-4 font-sans text-xs">
                <div className="flex flex-row gap-4">
                    <span className="text-text-muted">Vinnare: <span className="text-text">Qiiwi Games</span> <span className="market-positive font-semibold">+70,50%</span></span>
                </div>
                <div className="flex flex-row items-center gap-1.5 text-text-muted">
                    <span className="w-2 h-2 bg-accent rounded-full animate-pulse"></span> LIVE
                </div>
            </div>
                        ...
                        <div className="flex flex-row flex-wrap items-center gap-x-2 gap-y-1 mb-1 font-sans text-[11px]">
                            ...
                            {item.reaction && (
                                <span
                                    ...
                                    className={`font-semibold ${item.reaction.startsWith("-") ? "market-negative" : "market-positive"}`}
                                >
                                    {item.reaction}
                                </span>
                            )}
        ...
        <div className={`${card} p-6`} aria-hidden="true">
            ...
            <svg viewBox={`0 0 ${width} ${height + 24}`} className="w-full h-auto block" aria-hidden="true">
                {years.map((row, idx) => {
                    ...
                    return (
                        <g key={row.year}>
                            <rect x={base} y={scaleY(row.revenue)} width={barWidth} height={height - scaleY(row.revenue)} fill="var(--color-primary)" />
                            <rect x={base + barWidth + 4} y={scaleY(row.ebit)} width={barWidth} height={height - scaleY(row.ebit)} fill="var(--color-secondary)" />
                            <rect x={base + (barWidth + 4) * 2} y={scaleY(row.third)} width={barWidth} height={height - scaleY(row.third)} fill="var(--company-fin-ebita)" />
                            <text x={idx * groupWidth + groupWidth / 2} y={height + 18} textAnchor="middle" fill="var(--color-text-muted)" fontSize="12" fontFamily="sans-serif">
                                {row.year}
                            </text>
                        </g>
                    );
                })}
                <path d={marginPath} fill="none" stroke="var(--company-muted-line)" strokeWidth="2" />
                {years.map((row, idx) => (
                    <circle key={idx} cx={idx * groupWidth + groupWidth / 2} cy={height - row.margin * height * 0.9 - 20} r="3.5" fill="var(--company-muted-line)" />
                ))}
            </svg>
            <div className="flex flex-row flex-wrap gap-x-4 gap-y-1 mt-3 font-sans text-[11px] text-text-muted">
                ...
                <span><span className="demo-swatch demo-swatch--ebita inline-block w-2.5 h-2.5 mr-1.5 align-middle"></span>EBITA</span>
                <span><span className="demo-swatch demo-swatch--margin inline-block w-4 h-0.5 mr-1.5 align-middle"></span>EBIT-marginal</span>
            </div>
        </div>
    );
}
